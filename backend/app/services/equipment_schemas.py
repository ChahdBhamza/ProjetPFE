...
def schema_as_prompt_fields(equipment_type: str) -> str:
    ...
    hints: dict[str, dict[str, str]] = {
        "airconditioner": {
            ...
            "annual_energy_consumption_kwh": "float kWh PER YEAR. The scraped text may be in French or English — detect the unit period and convert: 'par mois'/'mensuelle'/'monthly'/kWh/mois → ×12 | 'par jour'/'journalière'/'daily'/kWh/jour → ×365 | 'par heure'/'horaire'/'hourly'/kWh/h → ×8760. Always return the annual (yearly) total.",
            "dimensions":     "actual HxWxD in cm e.g. \"29x79x19 cm\"",
        ...
        "refrigerator": {
            ...
            "dimensions":      "actual HxWxD in cm e.g. \"185x60x65 cm\"",
            "weight_kg":       "float kg",
            "noise_level_db":  "integer dB",
            "power_consumption_w": "integer Watts",
            "annual_energy_consumption_kwh": "float kWh PER YEAR. The scraped text may be in French or English — detect the unit period and convert: 'par mois'/'mensuelle'/'monthly'/kWh/mois → ×12 | 'par jour'/'journalière'/'daily'/kWh/jour → ×365 | 'par heure'/'horaire'/'hourly'/kWh/h → ×8760. Always return the annual (yearly) total.",
        ...
        "microwave": {
            ...
            "annual_energy_consumption_kwh": "float kWh PER YEAR. The scraped text may be in French or English — detect the unit period and convert: 'par mois'/'mensuelle'/'monthly'/kWh/mois → ×12 | 'par jour'/'journalière'/'daily'/kWh/jour → ×365 | 'par heure'/'horaire'/'hourly'/kWh/h → ×8760. Always return the annual (yearly) total.",
            "capacity_liters":       "integer liters",
            "functions":             "array of strings e.g. [\"Grill\", \"Defrost\", \"Convection\"]",
            "turntable_diameter_cm": "integer cm",
            "control_type":          "\"Digital\" | \"Analog\" | \"Touch\"",
            "dimensions":            "actual HxWxD in cm e.g. \"30x52x40 cm\"",
            "weight_kg":             "float kg",
            "warranty_years":        "integer",
        },
        "laptop": {
            "cpu":               "full CPU name e.g. \"Intel Core i7-12700H\"",
            "ram_gb":            "integer GB",
            "storage":           "string e.g. \"512GB SSD\"",
            "display_inches":    "float e.g. 15.6",
            "display_resolution":"string e.g. \"1920x1080\"",
            "gpu":               "GPU name string e.g. \"NVIDIA RTX 3060\"",
            "battery_wh":        "float Wh",
        ...
        "monitor": {
            "screen_size_inches":  "float screen size in inches e.g. 23.8",
            "resolution":          "string e.g. \"1920x1080\"",
            "panel_type":          "\"IPS\" | \"VA\" | \"TN\" | \"OLED\"",
            "refresh_rate_hz":     "integer e.g. 60 or 144",
            "response_time_ms":    "float ms e.g. 4.0",
            "aspect_ratio":        "string e.g. \"16:9\"",
            "brightness_cdm2":     "integer cd/m² e.g. 250",
            "contrast_ratio":      "string e.g. \"1000:1\"",
            "ports":               "array of strings e.g. [\"HDMI\", \"DisplayPort\"]",
            "power_consumption_w": "integer Watts",
            "warranty_years":      "integer",
        },
    }
